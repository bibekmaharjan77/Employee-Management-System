...
if(isset($_POST['Submit'])){
        ...
        $sql="UPDATE emp_record SET ename='$Ename', dept='$Dept', contact='$Contact', ban='$BAN', salary='$Salary',
                homeaddress='$HomeAddress' WHERE id='$search_query_parameter'";
        
        $execute=$connecting_db->query($sql);

        if($execute){
        //     //using js to send the user back to view wala table ko location
        echo '<script> window.open("index.php?id=Record Updated Successfully","_self") </script>';
